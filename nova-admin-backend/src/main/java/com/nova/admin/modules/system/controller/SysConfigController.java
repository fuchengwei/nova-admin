...
import com.nova.admin.modules.system.dto.StorageConnectionCheckRequest;
...
public class SysConfigController extends BaseController {
    ...
    @PostMapping("/upload/verify-storage")
    @PreAuthorize("hasRole('super_admin') or hasAuthority('system:settings:edit')")
    @Operation(summary = "检测文件存储连接")
    public R<Void> verifyUploadStorage(
            @Parameter(description = "文件存储连接检测请求", required = true)
            @Valid @RequestBody
            @io.swagger.v3.oas.annotations.parameters.RequestBody(description = "文件存储连接检测请求")
            StorageConnectionCheckRequest request) {
        fileService.verifyStorage(request.getStorageType());
        return ok();
    }
