...
- **消息草稿与定时发布**：消息发布支持立即发送、保存草稿和指定时间发送；发布记录展示生命周期状态、计划时间及失败原因，草稿可继续编辑或删除，待发送消息可取消，定时发送前会重新解析当前启用收件人。
- **消息发布记录与送达明细**：消息发布页新增发布记录与详情抽屉，可追踪发布人、接收人数、已读/未读统计和分页收件明细；手动发布会记录发布人，系统任务消息兼容显示为系统。
- **接口权限多主体授权**：已注册接口权限支持所有已登录用户、指定角色和指定用户三类可组合授权；授权保存后会刷新受影响用户的缓存权限与 SSE 会话状态。
...
- **消息发布前校验**：发布页会按当前接收范围预览实际启用用户数量和示例接收人；预览失败或没有可接收用户时自动禁止发布。
