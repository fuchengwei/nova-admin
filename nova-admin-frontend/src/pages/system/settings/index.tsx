...
import { Alert, Avatar, Button, Skeleton, Tag, message } from 'antd';
...
export default function SystemSettingsPage() {
  ...
  const canEdit = roles.includes('super_admin') || hasPermission('system:settings:edit', permissions);
  ...
  const { data: basicData, isLoading: basicLoading, refetch: refetchBasic } = useQuery({
  ...
  const { data: noticeData, isLoading: noticeLoading, refetch: refetchNotice } = useQuery({
  ...
  const safeBasicLogoSrc = useMemo(() => normalizeImageSrc(basicData?.logoUrl), [basicData?.logoUrl]);
  ...
  return (
    <PageContainer title={t('settings.title')} className="overflow-y-auto page-fill">
      <div className="h-full overflow-y-auto pb-6">
        <div className="space-y-6 pr-1">
          {!canEdit ? (
            <Alert className="rounded-2xl" type="info" showIcon title={t('settings.readonlyHint')} />
          ) : null}
          ...
          <div className="relative overflow-hidden rounded-[32px] bg-slate-950 px-6 py-7 text-white">
            ...
            <div className="relative grid gap-6 xl:grid-cols-[1.2fr_0.8fr] xl:items-end">
              <div>
                <div className="text-[11px] uppercase tracking-[0.34em] text-white/55">{t('settings.overviewLabel')}</div>
                <h1 className="mt-3 text-3xl font-semibold tracking-tight text-white">{t('settings.title')}</h1>
                <p className="mt-3 max-w-2xl text-sm leading-7 text-white/72">{t('settings.subtitle')}</p>
              </div>
          ...
          <div className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-4">
            <OverviewCard
              ...
              action={
                canEdit ? (
                  <Button type="text" icon={<EditOutlined />} onClick={() => setBasicModalOpen(true)}>
              ...
              highlights={[
                ...
                { label: t('settings.defaultLanguage'), value: displayText(basicData?.defaultLanguage) },
                { label: t('settings.themeColor'), value: displayText(basicData?.themeColor) },
                { label: t('settings.logoPreview'), value: safeBasicLogoSrc ? t('menu.enabled') : '-' },
              ]}
            />
            ...
            <OverviewCard
              ...
              action={
                canEdit ? (
                  <Button type="text" icon={<EditOutlined />} onClick={() => setSecurityModalOpen(true)}>
                    {t('settings.editSection')}
                  </Button>
                ) : null
              }
              highlights={[
                { label: t('settings.passwordMinLength'), value: displayText(securityData?.passwordMinLength) },
                {
                  label: t('settings.captchaEnabled'),
                  value: badge(Boolean(securityData?.captchaEnabled), t('menu.enabled'), t('menu.disabled')),
                },
                { label: t('settings.loginLockMaxAttempts'), value: displayText(securityData?.loginLockMaxAttempts) },
                { label: t('settings.loginLockMinutes'), value: displayText(securityData?.loginLockMinutes) },
              ]}
            />
            ...
            <OverviewCard
              ...
              action={
                canEdit ? (
                  <Button type="text" icon={<EditOutlined />} onClick={() => setUploadModalOpen(true)}>
                    {t('settings.editSection')}
                  </Button>
                ) : null
              }
              highlights={[
                { label: t('settings.maxSizeMb'), value: `${displayText(uploadData?.maxSizeMb)} MB` },
                { label: t('settings.avatarMaxSizeMb'), value: `${displayText(uploadData?.avatarMaxSizeMb)} MB` },
                { label: t('settings.allowedTypes'), value: displayText(uploadData?.allowedTypes) },
                { label: t('settings.avatarAllowedTypes'), value: displayText(uploadData?.avatarAllowedTypes) },
              ]}
            />
            ...
            <OverviewCard
              ...
              action={
                canEdit ? (
                  <Button type="text" icon={<EditOutlined />} onClick={() => setNoticeModalOpen(true)}>
                    {t('settings.editSection')}
                  </Button>
                ) : null
              }
              highlights={[
                ...
                },
                {
                  ...
                  value: badge(Boolean(noticeData?.emailEnabled), t('menu.enabled'), t('menu.disabled')),
                },
                {
                  label: t('settings.smsEnabled'),
                  value: badge(Boolean(noticeData?.smsEnabled), t('menu.enabled'), t('menu.disabled')),
                },
              ]}
            />
          </div>
          ...
          <div className="flex flex-col gap-6">
            <DetailSection
              ...
              action={
                canEdit ? (
                  <Button type="primary" icon={<EditOutlined />} onClick={() => setBasicModalOpen(true)}>
                    {t('settings.editSection')}
                  </Button>
                ) : null
              }
            >
              <div className="grid gap-4 lg:grid-cols-[260px_1fr]">
                <div className="rounded-[24px] bg-slate-50 p-5 text-center">
                  <Avatar shape="square" size={104} src={safeBasicLogoSrc} icon={<SettingOutlined />} />
                  <div className="mt-4 text-lg font-semibold text-slate-900">{displayText(basicData?.systemName)}</div>
                  <div className="mt-1 text-sm text-slate-500">{displayText(basicData?.browserTitle)}</div>
                </div>
                <div className="grid gap-4 md:grid-cols-2">
                  <MetaPanel title={t('settings.basicTab')}>
                    <MetaRow label={t('settings.systemName')} value={displayText(basicData?.systemName)} />
                    <MetaRow label={t('settings.browserTitle')} value={displayText(basicData?.browserTitle)} />
                    <MetaRow label={t('settings.defaultLanguage')} value={displayText(basicData?.defaultLanguage)} />
                  </MetaPanel>
                  <MetaPanel title={t('settings.logoPreview')}>
                    <MetaRow label={t('settings.themeColor')} value={displayText(basicData?.themeColor)} />
                    <MetaRow label={t('settings.copyrightText')} value={displayText(basicData?.copyrightText)} />
                  </MetaPanel>
                </div>
              </div>
            </DetailSection>
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <MetaPanel title={t('settings.passwordPolicy')}>
                  <MetaRow label={t('settings.passwordMinLength')} value={displayText(securityData?.passwordMinLength)} />
                  <MetaRow
                    label={t('settings.passwordRequireNumber')}
                    value={badge(Boolean(securityData?.passwordRequireNumber), t('menu.yes'), t('menu.no'))}
                  />
                  <MetaRow
                    label={t('settings.passwordRequireLetter')}
                    value={badge(Boolean(securityData?.passwordRequireLetter), t('menu.yes'), t('menu.no'))}
                  />
                  <MetaRow
                    label={t('settings.passwordRequireSpecial')}
                    value={badge(Boolean(securityData?.passwordRequireSpecial), t('menu.yes'), t('menu.no'))}
                  />
                </MetaPanel>
                <MetaPanel title={t('settings.loginProtection')}>
                  <MetaRow
                    ...
                    value={badge(Boolean(securityData?.captchaEnabled), t('menu.enabled'), t('menu.disabled'))}
                  />
                  <MetaRow label={t('settings.loginLockMaxAttempts')} value={displayText(securityData?.loginLockMaxAttempts)} />
                  <MetaRow label={t('settings.loginLockMinutes')} value={displayText(securityData?.loginLockMinutes)} />
                  <MetaRow
            ...
            >
              <div className="grid gap-4 md:grid-cols-2">
                <MetaPanel title={t('settings.generalUpload')}>
                  <MetaRow label={t('settings.maxSizeMb')} value={`${displayText(uploadData?.maxSizeMb)} MB`} />
                  <MetaRow label={t('settings.allowedTypes')} value={displayText(uploadData?.allowedTypes)} stacked />
                </MetaPanel>
                <MetaPanel title={t('settings.avatarUploadGroup')}>
                  <MetaRow label={t('settings.avatarMaxSizeMb')} value={`${displayText(uploadData?.avatarMaxSizeMb)} MB`} />
                  <MetaRow label={t('settings.avatarAllowedTypes')} value={displayText(uploadData?.avatarAllowedTypes)} stacked />
                </MetaPanel>
              </div>
            </DetailSection>
            ...
            >
              <div className="grid gap-4 lg:grid-cols-[1.2fr_0.8fr]">
                <MetaPanel title={t('settings.noticeMessage')}>
                  <MetaRow label={t('settings.noticeTitle')} value={displayText(noticeData?.title)} />
                  <MetaRow
                ...
                <div className="grid gap-4">
                  <MetaPanel title={t('settings.emailChannel')}>
                    <MetaRow
                      ...
                      value={badge(Boolean(noticeData?.emailEnabled), t('menu.enabled'), t('menu.disabled'))}
                    />
                    <MetaRow label={t('settings.emailHost')} value={displayText(noticeData?.emailHost)} />
                    <MetaRow label={t('settings.emailPort')} value={displayText(noticeData?.emailPort)} />
                    <MetaRow label={t('settings.emailUsername')} value={displayText(noticeData?.emailUsername)} />
                  </MetaPanel>
                  <MetaPanel title={t('settings.smsChannel')}>
                    <MetaRow
                      ...
                      value={badge(Boolean(noticeData?.smsEnabled), t('menu.enabled'), t('menu.disabled'))}
                    />
                    <MetaRow label={t('settings.smsProvider')} value={displayText(noticeData?.smsProvider)} />
                  </MetaPanel>
                </div>
              </div>
            </DetailSection>
