...
export default function DictPage() {
  ...
  const dataColumns: ProColumns<DictDataRecord>[] = [
    ...
    },
    {
      ...
      search: false,
      render: (_, r) =>
        r.defaultFlag === 1 ? <Tag color="blue">{t('dict.yes')}</Tag> : <Tag>{t('dict.no')}</Tag>,
    },
    {
      title: t('dict.createTime'),
      dataIndex: 'createTime',
  ...
  return (
    <div className="flex h-full min-h-0 flex-col">
      <h2 className="mb-4 text-lg font-semibold">{t('menu.dict')}</h2>
                ...
                <>
                  ...
                  <ModalForm
                    ...
                    onFinish={async (values) => {
                      const res =
                        typeEditMode && editingType
                          ...
                          : await createTypeMutation.mutateAsync(
                              values as unknown as DictTypeCreateRequest,
                            );
                      if (res.code !== 0) {
                      ...
                      message.success(
                        typeEditMode ? t('dict.updateSuccess') : t('dict.createSuccess'),
                      );
                ...
                <>
                  ...
                  <ModalForm
                    ...
                    initialValues={
                      dataEditMode && editingData
                        ? {
                            ...
                            defaultFlag: editingData.defaultFlag,
                          }
                    ...
                    onFinish={async (values) => {
                      ...
                      const res =
                        dataEditMode && editingData
                          ? await updateDataMutation.mutateAsync({
                              id: editingData.id,
                              ...payload,
                            } as unknown as DictDataUpdateRequest)
                          : await createDataMutation.mutateAsync(
                              payload as unknown as DictDataCreateRequest,
                            );
                      if (res.code !== 0) {
                        message.error(res.msg || t('common.error'));
                        return false;
                      ...
                      message.success(
                        dataEditMode ? t('dict.updateSuccess') : t('dict.createSuccess'),
                      );
